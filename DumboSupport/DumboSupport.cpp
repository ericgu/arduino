...
void DumboData::Init()
{
	m_pItems = (DumboItem*) malloc( m_itemCount * sizeof(DumboItem));
	m_currentItem = 0;
	m_active = 1;
}
...
void TimerHandler::HandleInterrupt()
{
	...
	if (m_digitIndex == 4)
	{
		...
		if (m_repeatCount == REPEAT_COUNT_MAX)
		{
			m_repeatCount = 0;
